# -*- coding: utf-8 -*-
"""
Created on Wed Jun 10 22:59:35 2020
...
class Eges(tf.Module):
    ...
    def __init__(self,
        ...
        self.A = tf.Variable(tf.random.truncated_normal([self.feature_sizes[0],self.feature_num], mean=0, stddev=1, dtype=tf.dtypes.float32)
                             , name = 'A')   # |V| * feature_num
    ...
    def set_si_vars(self):
        ...
        for i in range(self.feature_num):
           si_all.append(tf.Variable(tf.random.truncated_normal([self.feature_sizes[i],self.embedding_size], mean=0, stddev=1, dtype=tf.dtypes.float32),
                                     name='side_information_'+str(i)))
        return si_all
    ...
    def train(self, xs, samples, ys, epoches):
        variables = [w for w in self.side_ws] + [self.A]
...
# 方案2, 利用tensorflow官方文档的w2v的损失函数, 自动实现了negative sampling
class Eges_v2(tf.Module):
    ...
    def __init__(self,
        ...
        self.A = tf.Variable(tf.random.truncated_normal([self.feature_sizes[0],self.feature_num], mean=0, stddev=1, dtype=tf.dtypes.float32),
                             name = 'A')   # |V| * feature_num
        #
        self.softmax_weights = tf.Variable(tf.random.truncated_normal([self.feature_sizes[0],embedding_size], mean=0, stddev=1, dtype=tf.dtypes.float32),
                                           name='soft_w')
        self.softmax_biases = tf.Variable(tf.zeros(self.feature_sizes[0]),
                                           name='soft_b')
